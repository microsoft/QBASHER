	...
	*p = 0;
	...
	if (qoenv->report_match_counts_only) {
		...
		while (shown < qoenv->max_to_show && i < qex->tl_returned) {
			...
			if (0) printf("copying to lrr[%d / %d]: %s\n", shown, qoenv->max_to_show, (char *)(qex->tl_suggestions[i]));
...
//#define MAX_OS_RESULT_LEN 16384 //Author2
#define MAX_OS_RESULT_LEN 8192 //Author2
